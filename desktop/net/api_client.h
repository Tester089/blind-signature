...
public:
    ...
    void createPoll(const QString &title, const QStringList &candidates, int tokenCount,
                    const QString &ownerUsername = {}, const QStringList &allowedUsernames = {});
    void issueToken(const QString &pollId, const QString &username, int count = 1);
    void signBlinded(const QString &pollId, const QString &token, const QString &blindedMessage);
    void submitVote(const QString &pollId, const QString &ballot, const QString &signature,
                    const QString &token, const QString &username);
    void getResults(const QString &pollId);
    void closePoll(const QString &pollId, const QString &username);
...
signals:
    ...
    void requestFailed(const QString &where, int httpStatus, const QString &errorCode, const QString &message);

private:
    void handleJsonReply(QNetworkReply *reply, const QString &where,
