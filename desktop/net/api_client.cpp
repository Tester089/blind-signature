...
QUrl ApiClient::makeUrl(const QString& path) const {
    ...
    url.setPath(QStringLiteral("/") + clean);
    return url;
}
...
void ApiClient::listPolls() {
    ...
    handleJsonReply(reply, "listPolls", [this](const QJsonObject& obj) {
        ...
        const auto arr = obj.value("polls").toArray();
...
void ApiClient::createPoll(const QString& title,
                           const QStringList& candidates,
                           int tokenCount,
                           const QString& ownerUsername,
                           const QStringList& allowedUsernames) {
    ...
    QJsonArray allowed;
    for (const auto& u : allowedUsernames) {
        allowed.push_back(u);
    }
    ...
    if (!ownerUsername.trimmed().isEmpty()) {
        body["owner_username"] = ownerUsername.trimmed();
    }
    if (!allowedUsernames.isEmpty()) {
        body["allowed_usernames"] = allowed;
    }
...
void ApiClient::issueToken(const QString& pollId,
...
void ApiClient::signBlinded(const QString& pollId,
                            const QString& token,
...
void ApiClient::submitVote(const QString& pollId,
...
void ApiClient::closePoll(const QString& pollId, const QString& username) {
...
void ApiClient::handleJsonReply(QNetworkReply* reply,
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, where, onSuccess]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            QString errorCode = "network_error";
            QString message = reply->errorString();

            if (parseResult.isObject()) {
                const auto obj = parseResult.object();
...
PollSummary ApiClient::parsePollSummary(const QJsonObject& obj) {
    ...
    p.ownerUsername = obj.value("owner_username").toString();
    p.allowedVotersCount = obj.value("allowed_voters_count").toInt();
    return p;
}
...
PollDetails ApiClient::parsePollDetails(const QJsonObject& obj) {
    ...
    p.ownerUsername = obj.value("owner_username").toString();
    p.allowedUsernames = jsonArrayToStringList(obj.value("allowed_usernames").toArray());
    p.votedUsernamesCount = obj.value("voted_usernames_count").toInt();
    return p;
}
...
CreatePollResponse ApiClient::parseCreatePollResponse(const QJsonObject& obj) {
    ...
    r.ownerUsername = obj.value("owner_username").toString();
    r.allowedUsernames = jsonArrayToStringList(obj.value("allowed_usernames").toArray());
    return r;
}
